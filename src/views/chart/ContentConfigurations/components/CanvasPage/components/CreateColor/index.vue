<template>
  <n-modal class="go-chart-create-color" v-model:show="modelShowRef" :mask-closable="false" :closeOnEsc="false">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true" style="width: 900px; height: 720px">
      ...
      <div class="create-content">
        <div class="create-color-setting-box">
          ...
          <div v-else class="no-data go-flex-center">
            ...
            <n-text :depth="3">No custom color selected yet</n-text>
          </div>
        </div>
      ...
      <template #action>
        <n-space justify="end">
          <n-button @click="closeHandle">Operation completed</n-button>
        </n-space>
      </template>
    </n-card>
  </n-modal>
...
const selectHandle = (item: ColorType) => {
  ...
  if (updateColor.value !== undefined) {
    goDialog({
      message: 'If there are changes that have not been saved, whether to discard the modifications？',
      onPositiveCallback: () => {
...
const deleteHandle = (index: number) => {
  ...
  if (updateColor.value !== undefined) {
    ...
    goDialog({
      message: `Whether to delete this color？`,
      onPositiveCallback: () => {
        positiveHandle()
      }
    })
...
const saveHandle = (onMessage = true) => {
  ...
  if (index !== -1) {
    onMessage && window.$message.success('The data application is successful！')
    const updateColorPrefix = cloneDeep({ ...updateColor.value, name: updateColor.value.name || 'Undefined' })
    ...
    window.$message.error('The data application failed！')
  }
}
