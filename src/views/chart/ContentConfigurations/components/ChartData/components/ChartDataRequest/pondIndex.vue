...
const closeAndSendHandle = () => {
  if (!props.targetDataRequest?.dataPondName) {
    window.$message.warning('Please enter a name in the bottom left corner！')
    inputInstRef.value?.focus()
    return
  }
  goDialog({
    message: 'Saving the content will modify all components that use this interface synchronously, whether to continue or not?',
    isMaskClosable: true,
