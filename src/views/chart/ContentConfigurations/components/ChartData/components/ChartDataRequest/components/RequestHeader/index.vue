  ...
  <n-space vertical>
    <div style="width: 600px">
      <n-tabs v-model:value="requestContentType" type="segment" size="small">
        <n-tab :name="RequestContentTypeEnum.DEFAULT" tab="Normal requests"> </n-tab>
      ...
      <div class="go-mt-3">
        ...
        <div v-else>
          ...
          <n-card class="go-mt-3 go-pb-3" v-if="requestParamsBodyType === RequestBodyEnum['NONE']">
            <n-text depth="3">The interface does not Body </n-text>
          </n-card>
    ...
    <div v-show="requestContentType === RequestContentTypeEnum.SQL">
      <template v-if="requestHttpType === RequestHttpEnum.GET">
        <n-text>SQL types are not supported Get request，please use other methods</n-text>
      </template>
      <template v-else>
        <n-tag type="warning">Specialized processing is required in the background sql Interface</n-tag>
