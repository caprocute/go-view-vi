<template>
  ...
  <div class="go-chart-data-pond">
    <n-card class="n-card-shallow">
      <setting-item-box name="The name of the request" :alone="true">
        <n-input size="small" :placeholder="pondData?.dataPondName || 'Not yet'" :disabled="true">
          <template #prefix>
      ...
      <setting-item-box name="Interface address" :alone="true">
        <n-input size="small" :placeholder="pondData?.dataPondRequestConfig.requestUrl || 'Not yet'" :disabled="true">
          <template #prefix>
...
const sendHandle = async () => {
  if (!targetData.value?.request) {
    window.$message.warning('Please select a public interface！')
    return
  }
  loading.value = true
  try {
    ...
    window['$message'].warning('I did not get the return value, please check the interface!')
  } catch (error) {
    console.error(error);
    loading.value = false
