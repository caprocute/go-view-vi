<template>
  <n-collapse-item title="Component interactions" name="1" v-if="interactActions.length">
    ...
    <div v-if="!targetData.events.interactEvents.length" class="no-data go-flex-center">
      ...
      <n-text :depth="3">No content yet</n-text>
    </div>
    ...
    >
      <n-space justify="space-between">
        <n-text>Associated components - {{ cardIndex + 1 }}</n-text>
      ...
      <n-tag :bordered="false" type="primary"> Select the target component </n-tag>

      <setting-item-box name="Trigger an event" :alone="true">
      ...
      <setting-item-box :alone="true">
        <template #name>
          ...
          <n-tooltip trigger="hover">
            ...
            <n-text>Not supported「static components」support「component」「public api」</n-text>
          </n-tooltip>
        </template>
        <n-select
          class="select-type-options"
          ...
          size="tiny"
          filterable
          placeholder="Only eligible components are showcased"
          v-model:value="item.interactComponentId"
          :options="fnEventsOptions()"
        />
      </setting-item-box>

      <setting-item-box v-if="fnDimensionsAndSource(item.interactOn).length" name="Query results" :alone="true">
        <n-table size="small" striped>
          <thead>
            <tr>
              <th v-for="item in ['Parameters', 'Illustrate']" :key="item">{{ item }}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <n-tag :bordered="false" type="primary"> Associate the target request parameters </n-tag>
...
const evDeleteEventsFn = (index: number) => {
  goDialog({
    message: 'Whether to delete this associated interaction module?',
    onPositiveCallback: () => {
