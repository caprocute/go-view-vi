<template>
  <n-collapse-item title="Advanced event configuration" name="3">
  ...
  <n-modal class="go-chart-data-monaco-editor" v-model:show="showModal" :mask-closable="false">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true" style="width: 1200px; height: 700px">
      <template #header>
        <n-space>
          <n-text>Advanced event editor（Use with source code）</n-text>
        </n-space>
      </template>
      ...
      <n-layout has-sider sider-placement="right">
        ...
        >
          <n-tabs default-value="1" justify-content="space-evenly" type="segment">
            <!-- 验证结果 -->
            <n-tab-pane tab="Verify the results" name="1" size="small">
                ...
                <n-collapse class="go-px-3" arrow-placement="right" :default-expanded-names="[1, 2, 3]">
                  <template v-for="error in [validEvents()]" :key="error">
                    <n-collapse-item title="Error function" :name="1">
                      <n-text depth="3">{{ error.errorFn || 'Not yet' }}</n-text>
                    </n-collapse-item>
                    <n-collapse-item title="ERROR MESSAGE" :name="2">
                      <n-text depth="3">{{ error.name || 'Not yet' }}</n-text>
                    </n-collapse-item>
                    <n-collapse-item title="Stack information" :name="3">
                      <n-text depth="3">{{ error.message || 'Not yet' }}</n-text>
                    </n-collapse-item>
                  </template>
                </n-collapse>
              </n-scrollbar>
            ...
            <!-- 辅助说明 -->
            <n-tab-pane tab="Description of the variable" name="2">
              <n-scrollbar trigger="none" style="max-height: 505px">
                <n-collapse class="go-px-3" arrow-placement="right" :default-expanded-names="[1, 2, 3, 4]">
                  <n-collapse-item title="e" :name="1">
                    <n-text depth="3">The parameter that is received when the corresponding lifecycle event is triggered</n-text>
                  </n-collapse-item>
                  <n-collapse-item title="this" :name="2">
                    <n-text depth="3">Chart component instance</n-text>
                    <br />
                  ...
                  <n-collapse-item title="components" :name="3">
                    <n-text depth="3"
                      >The collection IDs of all components on the dashboard The configuration ID in the chart component，other chart components can be obtained for control</n-text
                    >
                    <n-code :code="`{\n  [id]: component\n}`" language="typescript"></n-code>
                  </n-collapse-item>
                  <n-collapse-item title="node_modules" :name="4">
                    <n-text depth="3">The following are the package variables that are built into the code environment and available</n-text>
                    <br />
            ...
            <!-- 介绍案例 -->
            <n-tab-pane tab="Introduce the case" name="3">
              <n-scrollbar trigger="none" style="max-height: 505px">
                <n-collapse arrow-placement="right">
                  <n-collapse-item
                    v-for="(item, index) in templateList"
                    :key="index"
                    :title="`Case${index + 1}：${item.description}`"
                    :name="index"
                  >
      ...
      <template #action>
        <n-space justify="space-between">
          <div class="go-flex-items-center">
            ...
            <n-text class="go-ml-2" depth="2">The provided parameters allow you to add a custom tooltip to the chart、interactive events and more</n-text>
          </div>
...
const EventLifeName = {
  [EventLife.VNODE_BEFORE_MOUNT]: 'Before rendering',
  [EventLife.VNODE_MOUNTED]: 'After rendering'
}

const EventLifeTip = {
  [EventLife.VNODE_BEFORE_MOUNT]: 'Component at this time DOM it doesn t exist yet',
  [EventLife.VNODE_MOUNTED]: 'Component at this time DOM already exists'
}
...
const saveEvents = () => {
  if (validEvents().errorFn) {
    window['$message'].error('The event function is incorrect and cannot be saved')
    return
  }
