...
const chartModeList = [
  { label: 'List', icon: AlbumsIcon, value: ChartModeEnum.SINGLE },
  { label: 'Grid', icon: GridIcon, value: ChartModeEnum.DOUBLE }
]
...
const selectChartHandle = async (item: ConfigType) => {
  ...
  try {
    ...
    loadingError()
    window['$message'].warning(`Charts are in development, stay tuned...`)
  }
}
